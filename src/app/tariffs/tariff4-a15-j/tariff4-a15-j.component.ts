...
@Component({
	selector: 'app-tariff4-a15-j',
	templateUrl: './tariff4-a15-j.component.html',
	animations: [
...
export class Tariff4A15JComponent implements OnInit, OnDestroy {
	...
	eventRemove: boolean;
	removeIndex: any;
	displayedColumns: string[] = ['Concert Act', 'Venue', 'Attendance', 'Promoter Name', 'Promoter Address', 'Date', 'Gross Sales/ Fees (CAD$)', 'Performer Fee (CAD$)', 'menu'];
	calcForm: FormGroup;
	...
	constructor(private tariffDetailService: TariffDetailService, private reportApiServices: ReportApiServices, private selectTariffService: SelectTariffService, private router: Router, public canDeactivateService: CanDeactivateService, public dialogService: DialogService, @Inject(LOCALE_ID) public localeId: string, private route: ActivatedRoute, private reportCounterService: ReportCounterService) {
		...
		this.calcForm = new FormGroup({
			...
			'withRecorded': new FormControl(null, [Validators.min(1)])
		});
	...
	getMaxDate(date) {
		...
		if (date.year == this.currentYear) {
			...
			this.eventMaxDate = new Date(date.year, 11, 31);
		}
	}
	...
	setDetailValue(arr) {
		...
		arr.forEach((item) => {
			if (item.COMPANY == 'RESOUND' && item.NO_OF_EVENTS > 0) {
				this.calcForm.get('withRecorded').setValue(item.NO_OF_EVENTS);
			}
		});
	...
	calcTariff() {
		...
		let withRecorded = +this.calcForm.get('withRecorded').value;
		let totalTaxRs = 0;
		let gstRs = 0;
		let pstRs = 0;
		let hstRs = 0;
		...
		if (withRecorded && withRecorded > 0) {
			let totalRs = feeRs * withRecorded;
			this.taxApiVlaue.map((x) => {
			...
			this.gstFees += gstRs;
			this.pstFees += pstRs;
			totalTaxRs = totalRs + gstRs + pstRs + hstRs;
	...
	sendFees(step) {
		...
		let dataObj = {
			...
			withRecorded: this.calcForm.get('withRecorded').value
		};
